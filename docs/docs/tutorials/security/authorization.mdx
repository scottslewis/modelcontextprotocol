...
For our testing purposes, we will create an extremely simple MCP server that exposes just one tool - one that adds two numbers together. The server will require authorization to access this.
