...
The alternative would be for every customer or end user to provide _their own_ client registration, but that's just trading one complex task for another - now when a user connects to a MCP server, they need to go through their IT team to create a registration, assign it the right permissions, and then configure the MCP client to use it.
