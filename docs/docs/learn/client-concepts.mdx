...
Roots serve as a coordination mechanism between clients and servers, not a security boundary. They communicate intent and scope rather than enforce access control.  The specification requires servers to "SHOULD respect root boundaries," not "MUST enforce" them. This advisory nature means roots guide well-behaved servers to focus their operations appropriately, while clients remain responsible for actual access control.

**Why roots work this way**

Roots cannot be a security mechanism because servers run code the client cannot control. The specification acknowledges this reality by designing roots as advisory, placing all security requirements ("MUST") on clients. This design is pragmatic: **clients enforce security, roots communicate intent**.

**What roots are useful for**

Roots excel at coordination between cooperative parties:

- **Context scoping**: Telling servers "I'm working in this directory right now"
- **Accident prevention**: Helping well-behaved servers avoid operating in wrong locations
- **User intent signaling**: Communicating "these are the areas I care about"
- **Workflow organization**: Allowing clients to manage project boundaries automatically

**What roots are not useful for**

Roots do not provide:

- Protection against malicious servers
- Security guarantees or access control
- Substitutes for proper permission systems

**When to use roots**

Roots work best in environments where:

- Servers are trusted or vetted
- Users understand servers may ignore boundaries
- Clients implement their own access controls
- The goal is preventing accidents, not stopping malicious behavior
...
**Automatic root detection**: When users open folders, clients automatically expose them as roots. Opening a travel workspace allows the client to expose that directory as a root, helping servers understand which itineraries and documents are relevant to the current work.
