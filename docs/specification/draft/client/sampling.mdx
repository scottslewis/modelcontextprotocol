...
When using tool use in sampling, messages **MUST** be properly balanced: every assistant message containing `ToolUseContent` (with `id: $id`) must be followed by a user message containing corresponding `ToolResultContent` (with `toolUseId: $id`) before the next assistant message.

This requirement ensures:

- Tool uses are always resolved before the conversation continues
- Provider APIs (especially Claude API) can process parallel tool uses correctly
...
MCP allows models to use multiple tools in parallel (returning an array of `ToolUseContent`). All major provider APIs support this:
...
6. Clients **SHOULD** implement user approval for tool executions
7. Servers **SHOULD** validate tool results before using them
8. Clients **SHOULD** sanitize tool arguments before execution
9. Clients **SHOULD** implement timeouts for tool executions
10. Clients **MUST** validate that tool use IDs match tool result IDs
11. Both parties **SHOULD** implement iteration limits for tool loops
