...
- `inputSchema`: JSON Schema defining expected parameters
  ...
  - Defaults to draft-07 if no `$schema` field is present
...
#### Tool with default draft-07 schema:
...
#### Tool with explicit 2020-12 schema:
...
Note: The `dependentRequired` keyword is a 2020-12 feature that requires both `minPrice` and `maxPrice` to be present together if either is specified.
