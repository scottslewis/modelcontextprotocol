...
Authorization in the Model Context Protocol provides a secure way to protect access to sensitive resources and operations. When you've built an MCP server that accesses user data or performs administrative actions, you need to ensure only authorized users can access it.

Authorization in MCP provides a standardized way to build trust between AI systems and protected resources. Whether you're integrating with corporate systems, personal data stores, or any service that requires user consent, MCP's authorization flow gives you the tools to implement proper security.

## The Authorization System

MCP authorization works like a security system for protected resources. The system:

1. **Verifies identity** through a trusted authorization server
2. **Issues temporary passes** (access tokens) that prove authorization
3. **Validates those passes** every time you access something protected
4. **Automatically handles renewals** to maintain session continuity

MCP's approach builds on OAuth 2.1, a battle-tested standard that powers everything from Google's APIs to GitHub integrations. You're working with established industry best practices, not a proprietary system.

## When Should You Use Authorization?

Authorization in MCP is **optional**, but you should strongly consider it when:

- Your server accesses user-specific data (emails, documents, databases)
- You need to audit who performed which actions
- Your server integrates with third-party APIs that require user consent
...
**Important**: If you're using STDIO transport (like with Claude Desktop), stick with environment-based credentials instead. The OAuth flow is designed for HTTP-based transports where browsers can handle the authorization process.

For detailed specification information, see the [MCP Authorization specification](/specification/draft/basic/authorization).
...
When your MCP client first tries to connect, your server responds with a `401 Unauthorized` and tells the client where to find authorization information:

```http
HTTP/1.1 401 Unauthorized
WWW-Authenticate: Bearer realm="mcp", 
  resource_server_metadata_url="https://your-server.com/.well-known/oauth-protected-resource"
```
