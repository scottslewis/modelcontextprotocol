...
Authorization in the Model Context Protocol (MCP) secures access to sensitive resources and operations. If your MCP server handles user data or administrative actions, authorization ensures only permitted users can access protected endpoints.

MCP uses standardized authorization flows to build trust between AI systems and resources, supporting integrations with corporate systems, personal data stores, and any service requiring user consent. The protocol is based on OAuth 2.1, an industry standard powering APIs from Google, GitHub, and more—so you benefit from proven security practices.
...
Authorization in MCP is **optional**, but strongly recommended when:
...
**Important**: For STDIO transport you can use environment-based credentials or credentials provided by third-party libraries instead. OAuth flows are designed for HTTP-based transports where the MCP server is remotely-hosted.
